...
Latley I been investigating more on how to leave less finger prints in the internet,
and I might do a post in the future specifically about this. But one of the first 
things that I realized during my investigation is how easy it is to set a shared 
password store with my laptop and my other devices by using `pass` and `git`.

This post explains how to:

- Create your `pass` store and save it in a `git` repository
- Setup the same store in your Android device
- enable browser autofill

All this without third-party vault services

# Setup pass

# Syncing pass with Android over SSH
...
`pass` (the Unix password store) gives you exactly that: GPG-encrypted files in
